...
MSDevice_SSM::flushGlobalMeasures() {
    ...
    if (myComputeBR || myComputeSGAP || myComputeTGAP) {
        ...
        if (myComputeTGAP) {
            ...
            myOutputFile->openTag("FollowSpeedSpan").writeAttr("values", makeStringWithNAs(myFollowSpeedspan, INVALID)).closeTag();
            if (myMinTGAP.second != "") {
                if (myUseGeoCoords) {
